...
import { createDefaultTimeSlot, updateDefaultTimeSlot } from "@/redux/actions/timeSlotActions";
import { resetStatus } from "@/redux/slices/timeSlotSlice";


const EditDefaultTimeSlot = () => {
    const router = useRouter()
    const dispatch = useAppDispatch()
    const [showFromPicker, setShowFromPicker] = useState(false)
    const [showToPicker, setShowToPicker] = useState(false)
    const {loading, defaultTimeSlots, status} = useAppSelector(state => state.timeSlot)
    const {timeSlotId} = useLocalSearchParams()    
    const timeSlot = defaultTimeSlots.find(slot => slot.id === timeSlotId)

    const formik = useFormik({
        initialValues:{
            from: new Date(timeSlot!.fromFull),
            to: new Date(timeSlot!.toFull),
        },
        onSubmit:(formData) => {
            dispatch(updateDefaultTimeSlot({id: timeSlotId as string, formData}))
        },
    ...
    return (
        <SafeAreaView className="flex-1 items-center bg-white">
            <ScrollView showsVerticalScrollIndicator={false} className="w-[90%] pb-8">
...
export default EditDefaultTimeSlot;